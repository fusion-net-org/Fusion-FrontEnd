...
const OverviewCompanyPage: React.FC = () => {
  const [overview, setOverview] =
    useState<CompanyGrowthAndStatusOverview | null>(null);
  const [loadingOverview, setLoadingOverview] = useState(false);

  const [projectLoadOverview, setProjectLoadOverview] =
    useState<CompanyProjectLoadOverview | null>(null);
  ...
  useEffect(() => {
    ...
    async function fetchOverview() {
      ...
      try {
        ...
        message.error(
          error?.message || "Failed to load company growth & status overview!"
        );
      } finally {
  ...
  useEffect(() => {
    ...
    async function fetchProjectLoad() {
      ...
      try {
        ...
        message.error(
          error?.message || "Failed to load company project load distribution!"
        );
      } finally {
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <p className="max-w-xl text-xs text-gray-500">
                High-level overview of companies, activity status, growth
                trends, and project load distribution.
              </p>
            </div>
          </div>
      ...
      <Card
        ...
        extra={
          <div className="flex items-center gap-1 text-[11px] text-gray-500">
            ...
            <span>
              Distribution of total projects per company, bucketed by project
              count.
            </span>
          </div>
        }
        ...
        bodyStyle={{ padding: 16 }}
      >
        <CompanyProjectLoadChart
          overview={projectLoadOverview}
          loading={loadingProjectLoad}
        />
      </Card>
    </div>
  );
