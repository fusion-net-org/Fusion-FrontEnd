...
/* === avatar group ngắn gọn + overflow count === */
...
function Avatars({ list }: { list: TaskVm["assignees"] }) {
  const shown = list.slice(0, 3); const more = Math.max(0, list.length - shown.length);
  return (
    <div className="flex items-center">
      {shown.map((m, i) => <div key={m.id} className={i > 0 ? "-ml-2" : ""}><Avatar name={m.name} avatarUrl={m.avatarUrl} /></div>)}
      {!!more && <div className="-ml-2 w-6 h-6 rounded-full ring-2 ring-white bg-slate-300 text-[10px] flex items-center justify-center font-semibold text-slate-700">+{more}</div>}
    </div>
  );
}
...
export default function ProjectTaskList({
  ...
  // quick filter (cắt giảm): search + status pills + pagination
  const [kw, setKw] = useState("");
  const [statuses, setStatuses] = useState<Array<TaskVm["status"]>>([]);
  ...
  const StatusBtn = ({ v, label }: { v: TaskVm["status"]; label: string }) => {
    const active = statuses.includes(v);
    return (
      <button
        onClick={() =>
          setStatuses((s) => (active ? s.filter((x) => x !== v) : [...s, v]))
        }
        className={cn(
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-3 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div className="hidden md:flex items-center gap-2">
            <StatusBtn v="todo" label="To do" />
            <StatusBtn v="inprogress" label="In progress" />
            <StatusBtn v="inreview" label="In review" />
            <StatusBtn v="done" label="Done" />
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {slice.map((t) => {
                ...
                return (
                  <tr key={t.id} className={cn("border-b last:border-0", t.status === "done" && "opacity-70")}>
                    {/* code + sprint */}
                    ...
                    {/* title + tags (chỉ ưu tiên priority + type) */}
                    <td className="px-3 py-3 align-top">
                    ...
                    {/* assignees (overflow count) */}
                    <td className="px-3 py-3 align-top">
                      <Avatars list={t.assignees || []} />
                    ...
                    <td className="px-3 py-3 align-top">
                      ...
                      <div className="text-[11px] text-slate-500 mt-1">
                        {Math.max(0, t.estimateHours - t.remainingHours)}/{t.estimateHours}h
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-3 align-top">
                      ...
                      {target != null && t.status !== "done" && (
                        ...
                        >
                          ...
                          {overdue ? `Overdue ${Math.abs(remain!)}h` : `SLA ${remain}h left`}
                        </div>
                      )}
                    </td>
