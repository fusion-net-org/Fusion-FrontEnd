...
const RejectReasonModal: React.FC<RejectReasonModalProps> = ({
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      // Gửi reason (có thể rỗng)
      const res = await RejectProjectRequest(projectId, reason || '');
    ...
    <Modal
      title={
        <div>
          ...
          <p className="text-sm text-gray-500">Optionally provide a reason for rejection</p>
        </div>
      }
      open={open}
      onCancel={onClose}
